...
log = logging.getLogger("biotwin-api")

# -----------------------------------------------------------------------------
#  CONFIG (NO hardcode passwords in production)
# -----------------------------------------------------------------------------
DB_USER = os.getenv("DB_USER", "root")
DB_PASS = os.getenv("DB_PASS", "")  # ✅ sin password por defecto
...
log.info("🚀 BioTwin AI GOLD MASTER V2.1 - DB: %s:%s/%s", DB_HOST, DB_PORT, DB_NAME)

# -----------------------------------------------------------------------------
#  Timestamp "EFECTIVO" (fix fecha_medicion congelada)
# -----------------------------------------------------------------------------
TS_EFFECTIVE_EXPR = """
CASE
...
def period_to_mysql_interval(period: str) -> str:
    # ✅ evita inyección (solo mapping permitido)
    return ALLOWED_PERIODS.get(period, "24 HOUR")

def serialize_ts(ts) -> str:
    if ts is None:
        return None
...
def get_piscina_dimensions(pool_id: int):
    ...
    try:
        with engine.connect() as conn:
            q = text("SELECT superficie_m2, profundidad_m, volumen_m3 FROM piscinas WHERE id = :pid")
        ...
        log.warning("⚠️ Error dimensiones piscina_id=%s: %s", pool_id, e)

    return length, width, depth

# -----------------------------------------------------------------------------
#  GLOBAL JSON ERROR HANDLERS (✅ evita HTML/Texto "Internal Server Error")
# -----------------------------------------------------------------------------
...
def handle_any_exception(e):
    # ✅ siempre JSON (esto arregla el crash del frontend al hacer res.json())
...
@app.route("/")
def health_check():
    return jsonify({"status": "online", "system": "BioTwin AI", "version": "2.1.0"}), 200
...
def get_sensor_detail(pool_id: int):
    ...
    ✅ FIX CRÍTICO:
    - Antes se usaba TS_EFFECTIVE_EXPR en WHERE con alias "ts" (ilegal en MySQL)
    - Ahora se usa subquery y se filtra por t.ts
    """
    ...
    column = SENSOR_COLUMNS.get(sensor_type)
    if not column:
        return jsonify({"error": f"Invalid sensor '{sensor_type}'"}), 400

    # ✅ solo valores permitidos (no injection)
    interval = period_to_mysql_interval(period)

    def fetch_series(col_name: str):
        # Nota: col_name e interval vienen de whitelist
        q = text(f"""
            SELECT t.ts AS ts, t.val AS val
            FROM (
                SELECT
            ...
            WHERE t.ts >= DATE_SUB(NOW(), INTERVAL {interval})
            ORDER BY t.ts ASC
        """)
        with engine.connect() as conn:
            rows = conn.execute(q, {"pid": pool_id}).fetchall()
        ...
        for r in rows:
            ...
            if ts is None:
                continue
            out.append({
                "timestamp": serialize_ts(ts),
                "value": float(val) if val is not None else 0.0
            })
        return out

    try:
        main_data = fetch_series(column)

        cross_data = {}
        for cs in cross_sensors:
            cs_col = SENSOR_COLUMNS.get(cs)
            if not cs_col:
                continue
            cross_data[cs] = fetch_series(cs_col)

        return jsonify({"main": main_data, "cross": cross_data}), 200

    except SQLAlchemyError as e:
        log.exception("❌ sensor-detail SQL error pool_id=%s sensor=%s: %s", pool_id, sensor_type, e)
...
def get_sensor_history(pool_id: int):
    ...
    try:
        ...
        history = {"oxigeno": [], "ph": [], "temperatura": [], "nitrato": [], "timestamps": []}

        for row in reversed(rows):  # cronológico
            ts = row[0]
            history["timestamps"].append(ts.strftime("%H:%M") if isinstance(ts, datetime) else "")
            history["oxigeno"].append(float(row[1]) if row[1] is not None else 0.0)
            ...
            history["nitrato"].append(float(row[4]) if row[4] is not None else 0.0)

        return jsonify(history), 200
    except SQLAlchemyError as e:
        log.exception("❌ biofloc/history SQL error pool_id=%s: %s", pool_id, e)
        return jsonify({"error": "Database error"}), 500
    ...
    try:
        with engine.connect() as conn:
            q = text(f"""
                ...
                LIMIT 1
            """)
            result = conn.execute(q, {"pid": pool_id}).fetchone()

        if not result:
            return jsonify({
                "ion_nitrato": 0,
                "carbon_demand": 0,
                "oxigeno_disuelto": 5.0,
                "ph": 7.0,
                "temperatura": 25.0,
        ...
        ph_val = float(result[2]) if result[2] is not None else 7.0
        temp = float(result[3]) if result[3] is not None else 25.0
        ts = result[4]

        if isinstance(ts, datetime):
            data_age = (datetime.now() - ts).total_seconds() / 60
            last_updated_iso = ts.isoformat()
        else:
            data_age = 9999
            last_updated_iso = serialize_ts(ts)
        ...
        return jsonify({
            ...
            "data_age_minutes": round(float(data_age), 1),
        }), 200

    except SQLAlchemyError as e:
        log.exception("❌ biofloc/status SQL error pool_id=%s: %s", pool_id, e)
        return jsonify({"error": "Database error"}), 500
...
def get_yesterday_comparison(pool_id: int):
    ...
    try:
        ...
        if result:
            nitrato_ayer = float(result[0]) if result[0] is not None else 0.0
            return jsonify({
                "turbidity_yesterday": min(nitrato_ayer / 100.0, 1.0),
                "color_hex_yesterday": calcular_color_agua(nitrato_ayer)
            }), 200

        return jsonify({"turbidity_yesterday": 0, "color_hex_yesterday": "#0EA5E9"}), 200

    except SQLAlchemyError as e:
        log.exception("❌ digital-twin/compare SQL error pool_id=%s: %s", pool_id, e)
        return jsonify({"error": "Database error"}), 500
...
def get_digital_twin_config(pool_id: int):
    ...
    try:
        ...
        log.exception("❌ digital-twin/config SQL error pool_id=%s: %s", pool_id, e)
        return jsonify({"error": "Database error"}), 500
    ...
    try:
        with engine.connect() as conn:
            q = text(f"""
                ...
                LIMIT 24
            """)
            df = pd.read_sql(q, conn, params={"pid": pool_id})
        ...
        df = df.dropna(subset=["ts", "oxigeno_disuelto"]).sort_values("ts")
        ...
        for i in range(1, 7):
            next_val = max(0.0, current_o2 - (0.15 * i))
        ...
        log.exception("❌ predict/oxygen SQL error pool_id=%s: %s", pool_id, e)
        return jsonify({"error": "Database error"}), 500
...
def get_system_health():
    """
    Salud del sistema:
    - last_seen lo tomamos de created_at (ingesta), que sí viene bien.
    ...
    try:
        with engine.connect() as conn:
            q_sensors = text("""
                ...
                FROM piscinas p
                LEFT JOIN parametro_aguas pa
                  ON p.id = pa.piscina_id AND pa.deleted_at IS NULL
            ...
            for row in rows:
                ...
                for h in range(24):
                    ...
                    has_data = any(
                        (d and d[0] and hour_start <= d[0] < hour_end)
                        for d in heartbeat_data
                    )
            ...
            for l in log_rows:
                logs.append({
                    ...
                    "message": f"{l[1]} ({l[2]})",
                    "timestamp": l[3].strftime("%H:%M") if isinstance(l[3], datetime) else "--:--"
                })
        ...
        return jsonify({
            ...
            "logs": logs,
            "critical_alerts_today": int(critical_count)
        }), 200

    except SQLAlchemyError as e:
        log.exception("❌ system/health SQL error: %s", e)
        return jsonify({"error": "Database error"}), 500
...
if __name__ == "__main__":
    # En prod, usa gunicorn/uwsgi. Esto es para dev.

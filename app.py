...
print(f"🚀 BioTwin AI Backend v2.2 (Column Name Fix). Host DB: {DB_HOST}")
...
def health_check():
    return jsonify({
        "status": "online",
        "system": "BioTwin AI v2.2",
        "fix": "Column names corrected (largo -> largo_m)"
    }), 200
...
def get_digital_twin_config(pool_id):
    try:
        with get_db_connection() as conn:
            # --- CORRECCIÓN AQUÍ ---
            # Antes: SELECT largo, ancho, profundidad ...
            # Ahora: SELECT largo_m, ancho_m, profundidad_m ...
            q_pool = text("SELECT largo_m, ancho_m, profundidad_m FROM piscinas WHERE id = :pid")
            ...
            # Usamos índices [0], [1], [2] para leer las columnas
            l = float(pool_data[0]) if pool_data and pool_data[0] else 20.0
            w = float(pool_data[1]) if pool_data and pool_data[1] else 10.0
            d = float(pool_data[2]) if pool_data and pool_data[2] else 1.5
